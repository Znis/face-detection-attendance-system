...
def recordNow():
    ...
    attendance_record = [final_name,time,status]
...
csv_name = root_dir+'/Attendance_Details/attendance_report' + str((str(datetime.now())).split(' ')[0]) + '.csv'
mark_attendance_obj = Mark_Attendance(csv_filename=csv_name)
if not os.path.exists(os.path.join(root_dir,csv_name)):
        mark_attendance_obj.write_csv_header(staff_name='Staff_Name',time='Time',status='Status')
